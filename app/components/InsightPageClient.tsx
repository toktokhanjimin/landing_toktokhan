...
export default function InsightPageClient({ initialItems }: { initialItems: InsightItem[] }) {
  ...
  const filtered = active === "전체" ? initialItems : initialItems.filter((i) => i.tag === active);
    ...
    <div style={{ background: "var(--bg)", color: "var(--fg-1)", minHeight: "100dvh" }}>
      ...
      <section className="ins-page-section" style={{ padding: "0 24px 120px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div className="ins-page-grid" style={{ display: "grid", gridTemplateColumns: "1fr", rowGap: 0 }}>
            {visible.map((it, i) => (
              <InsightCard key={it.id ?? i} item={it} />
            ))}
          </div>
    ...
    >
      <div className="ins-thumb" style={{
        ...
        {(() => {
          ...
          const src = item.thumbImg
            ?? (["log", "talk", "tech"].includes(cat) ? `/assets/${cat}.png` : null);
          return src ? (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12, paddingTop: 4 }}>
        <h3 style={{
          font: "var(--h6)", letterSpacing: "-.01em",
          color: "var(--fg-1)", margin: 0, whiteSpace: "pre-line",
        }}>
          {item.title}
        </h3>
        <p className="ins-excerpt" style={{
          font: "400 14px/1.6 var(--font-sans)",
          color: "rgba(10,10,10,.6)", margin: 0,
        }}>
          {item.excerpt}
        </p>
        <div style={{
          ...
          color: "rgba(10,10,10,.45)", paddingTop: 8,
        }}>
